...
describe("Login related tests",()=>{
    ...
    describe("For the patient",()=>{
        ...
        it("Login to the new account and then call /me endpoint with the token",async ()=>{
            const res = await request(app).post('/api/login/patient')
            ...
            const res2 = await request(app).get("/api/me").set("Authorization", `bearer ${accessToken}`)
            expect(res2.body).toHaveProperty("email",email)
            expect(res2.body).toHaveProperty("type","patient")

            const res3 = await request(app).post("/api/logout")
                .send({
    ...
    describe("For the doctor",()=>{
        ...
        it("Create a new account",async ()=>{
            const res =  await request(app).post('/api/doctor').send({
                ...userInfo,
        ...
        it("Login to the new account and then call /me endpoint with the token",async ()=>{
            const res = await request(app).post('/api/login/doctor')
                .set('User-Agent',"Test Runner")
                .send({
                    email,
                    password
