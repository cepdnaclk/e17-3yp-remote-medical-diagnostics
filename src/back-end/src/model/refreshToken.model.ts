...
refreshTokenSchema.statics.invalidate = async function (refreshToken: String) {
  const a = await this.updateOne(
    { refreshToken: refreshToken },
    { valid: false }
  ).exec();
  if (a.nModified !== 1)
    log.info(
      "token invalidation failed (token doesn't exist or already invalidated)"
    );
};

refreshTokenSchema.statics.addNewToken = async function (
  refreshToken: String,
  userAgent: String
) {
  try {
...
refreshTokenSchema.statics.isRefreshTokenValid = async function (
  refreshToken: string
) {
  try {
    ...
    log.error("Database: isRefreshTokenValid error");
    throw error;
  }
};
const model = mongoose.model<refreshTokenDocument, refreshTokenModel>(
  "refreshToken",
  refreshTokenSchema
);
